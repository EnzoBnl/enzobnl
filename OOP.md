...
|**[Behavioral]** *Delegation Pattern*|*"In software engineering, the delegation pattern is an object-oriented design pattern that allows object composition to achieve the same code reuse as inheritance."*|When a method delegates its work to an underlying held attribute. For example a `Body` can delegate walking to its legs: its `.walk()` method simply calling `this.legs.walk()`.|Useful to match **OCP** when we can't achieve behavior reuse through inheritance for some reason|

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3NzE4MTk0NjMsLTEwMTU0OTY4NDFdfQ
==
-->