...
|**[structural]** *Decorator Pattern*|*"Attaches additional responsibilities to an object dynamically keeping the same interface. Decorators provide a flexible alternative to subclassing for extending functionality"*|Prevent class explosion or violation of ISP  (Because of LSP, subclasses relies on super class useless attributes for them). Can be done by bridge if a relation 1-1 can be found (City has 1 color), but if it's 1-n, only decorator chaining can do that.).||
