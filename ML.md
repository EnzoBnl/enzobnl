...
Let's consider a use case of binary classification, being given a matrix $X\in R^{n_x\times m}$ of samples (as columns) and binary labels $y \in \{0, 1\}^m$. The logistic regression learning uses a gradient descent to fit parameters $w, b\in R^{n_x}$  so that the model prediction for the $i^{th}$ sample is $p(y_{(i)}|X) = {ŷ_{(i)}}^y_{(i)}.(1 - ŷ)^{1-y}$, with $ŷ=logit(w^T.X + b)$, with $logit: z \rightarrow \frac{1}{1+e^{-z}}$. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDA2ODI5MzMyLC00MTAzMzg0MzYsMzM5MD
